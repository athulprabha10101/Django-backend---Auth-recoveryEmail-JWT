...
# Importing various serializers
from account.serializers import (UserLoginSerializer, UserPasswordResetSerializer, 
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if user is not None:
                ...
                return Response({'errors': {'non_field_errors': ['Email or password invalid']}}, status=status.HTTP_404_NOT_FOUND)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

# API View for displaying user profile
